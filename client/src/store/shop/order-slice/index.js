    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(createNewOrder.fulfilled, (state, action) => {
                ...
                // ! Utilizing session Storage - Store OrderID - Further Used as input for API Call...
...
export const capturePayment = createAsyncThunk("/order/capturePayment",
    async ({ paymentId, payerId, orderId }) => {
        const response = await axios.post(
            "http://localhost:5000/api/v1/shop/order/capture",
            {
                paymentId,
                payerId,
                orderId,
...
export const getAllOrdersByUserId = createAsyncThunk("/order/getAllOrdersByUserId",
    async (userId) => {
        const response = await axios.get(
            `http://localhost:5000/api/v1/shop/order/list/${userId}`
        );
        return response.data;
    }
);
